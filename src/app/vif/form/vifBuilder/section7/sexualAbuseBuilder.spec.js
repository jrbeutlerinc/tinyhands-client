...
describe('SexualAbuseBuilder', () => {

    let builder;

    describe('when constructed with no vif', () => {
        beforeEach(() => {
            builder = new SexualAbuseBuilder();
        });
    ...
    describe('when constructed with vif', () => {
        ...
        beforeEach(() => {
            vif = {
                home_had_sexual_abuse_never: true,
                home_had_sexual_abuse_rarely: true,
                home_had_sexual_abuse_frequently: true
            }

            builder = new SexualAbuseBuilder(vif);
        });

        it('should set emotionalAbuse option values to match vif', () => {
            expect(builder.never).toEqual(vif.home_had_sexual_abuse_never);
            expect(builder.rarely).toEqual(vif.home_had_sexual_abuse_rarely);
            expect(builder.frequent).toEqual(vif.home_had_sexual_abuse_frequently);
        });
    });
